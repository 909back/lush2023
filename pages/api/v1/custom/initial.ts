...
const handler = async(req:NextApiRequest, res:NextApiResponse) => {
    if(req.method === 'GET'){
        ...
        const list = {
            c_name: char.name,
            body: {order:1, name:'body', src:char.body,width:char.body_w,y:char.body_y},
            head: {order:2, name:'head', src:char.head, width: char.head_w, y:char.head_y},
            face: {order:3, name:'face', src:char.face, width: char.face_w, y:char.face_y},
        }
